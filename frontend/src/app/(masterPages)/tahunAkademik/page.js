...
export default function TahunAkademikPage() {
    ...
    const [dataThAkademik, setDataThAkademik] = useState([]);
    const [selectedThAkademik, setSelectedThAkademik] = useState(null);
    ...
    const { searchTerm, setSearchTerm } = useSearch(initialData, setDataThAkademik, ['tahun', 'status']);
    const modalRef = useRef(null);
    const [error, setError] = useState(null);

    const fetchDataThAkademik = async () => {
        try {
            await apiClient.get('/tahunAkademik', { cache: 'force-cache' })
                .then((res) => {
                    setDataThAkademik(res.data.data);
    ...
    const handleEdit = (thAkademik) => {
        setIsEdit(true);
        setSelectedThAkademik(thAkademik)
        setFormData({ tahun: thAkademik.tahun, status: thAkademik.status });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await apiClient.post('/tahunAkademik', formData);
            setIsLoading(false);
            Swal.fire({
                title: 'Success!',
    ...
    const handleUpdate = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await apiClient.put(`/tahunAkademik/${selectedThAkademik.id}`, formData);
            setIsLoading(false);
            Swal.fire({
                title: 'Success!',
                text: response.data.message,
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await apiClient.delete(`/tahunAkademik/${id}`)
                    .then((res) => {
                        fetchDataThAkademik();
                        Swal.fire({
                            title: "Deleted!",
    ...
    return (
        <div className="p-4" >
            <h1 className="text-2xl font-bold">Management Tahun Akademik</h1>
                ...
                <div className="fixed inset-0 flex items-center justify-center z-50">
                    ...
                    <div ref={modalRef} className="relative bg-white rounded-lg shadow-lg p-6 max-w-md w-full dark:bg-gray-700">
                        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
                            {isEdit ? 'Edit Tahun Akademik' : 'Tambah Tahun Akademik'}
                        </h3>
                        <form className="space-y-4">
                            <div>
                                <label htmlFor="tahun" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tahun</label>
                                <input value={formData.tahun} onChange={handleInputChange} type="text" name="tahun" id="tahun" autoComplete="off" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="tahun" required />
                                {error?.tahun && <p className="text-red-500 text-sm mt-1">{error?.tahun}</p>}
                            </div>
            ...
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        ) : dataThAkademik.length > 0 ? (
                            dataThAkademik.map((item, index) => (
